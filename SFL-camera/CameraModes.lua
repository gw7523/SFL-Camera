...
--[[
    ...
    - Added explicit dependency checks for Quaternion.lua functions.
    - Enhanced logging in setWeldedWingCamera to debug camera tracking.
    - Updated date and version to 03 February 2025.
]]
...
-- Welded Wing Camera: Fixed position and orientation relative to aircraft
function setWeldedWingCamera(identifier, offset_local)
    ...
    if enableLogging then
        log.write("CameraModes", log.INFO, "setWeldedWingCamera: '" .. identifier .. "' Quaternion: w=" .. q_aircraft.w .. 
    ...
    if enableLogging then
        log.write("CameraModes", log.INFO, "setWeldedWingCamera: Camera Quaternion: w=" .. q_camera.w .. 
...
if enableLogging then
    log.write("CameraModes", log.INFO, "CameraModes.lua (v1.6) loaded successfully.")
end